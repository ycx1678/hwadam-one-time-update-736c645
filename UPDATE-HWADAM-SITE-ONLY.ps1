...
$PackageUrl = "https://raw.githubusercontent.com/ycx1678/hwadam-one-time-update-736c645/main/hwadam-source-cda66d1.zip"
$TemporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) "hwadam-site-only-update-$([guid]::NewGuid().ToString('N'))"
...
try {
  ...
  Write-Host "1/4 최신 사이트 파일을 받습니다."
  ...
  try {
    Write-Host "3/4 사이트를 빌드하고 다시 시작합니다."
    & npm.cmd run build
    ...
    Write-Host "4/4 로컬 사이트 응답을 확인합니다."
    $healthy = $false
  ...
  Write-Host "완료: 최신 사이트 코드가 미니PC에 적용되었습니다."
  Write-Host "기존 data 폴더, 관리자 설정, 솔라피 API 설정은 변경하지 않았습니다."
} finally {
